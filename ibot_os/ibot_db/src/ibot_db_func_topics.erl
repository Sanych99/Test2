...
add_node_to_topic(TopicName, NodeName, ServerName) -> ?DBG_INFO("add_node_to_topic -> ~p~n", [[TopicName, NodeName, ServerName]]),
  ...
  case ibot_db_func:get_from_mnesia(topic_info, TopicName) of
    {atomic, []} -> %?DBG_INFO("add_node_to_topic topic ~p not found...~n", [TopicName]),  ?DBG_INFO("add_node_to_topic format ~p ...~n", [Vals]),
...
get_topic_nodes(TopicName) ->
  ...
  case ibot_db_func:get_from_mnesia(topic_info, TopicName) of
    ...
    {atomic, []} -> [];
    {atomic, [TopicInfo]} -> ?DBG_MODULE_INFO("get_topic_nodes Topicinfo: ~p~n", [?MODULE, TopicInfo#topic_info.subscribeNodes]),
