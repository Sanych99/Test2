...
public:
	...
	ServiceTestResp(matchable_ptr message_elements) {
		message_elements->match(make_e_tuple(e_string(&therdParamResp), long(&secParamResp), e_string(&strParamResp)));
	}
	...
	virtual void send_service_response(mailbox_ptr mbox, std::string service_core_node, 
		...
		ServiceTestRespReq req(request_message_from_client);
		mbox->send(service_core_node, core_node_name, make_e_tuple(atom(response_service_message), e_string(service_method_name), atom(client_mail_box_name),
			...
			make_e_tuple(e_string(therdParamResp), long(secParamResp), e_string(strParamResp))
		));
	}

	e_tuple<boost::fusion::tuple<e_string, long, e_string> > get_tuple_message() {
		return make_e_tuple(e_string(therdParamResp), long(secParamResp), e_string(strParamResp));
	};
